...
> 本计划只面向**离线数据整理、人工导出文件解析、只读决策辅助**。不得用于自动登录、自动交易、绕过风控、批量探测接口。

---

## 0. 一句话结论

从 2026-05-12 这轮状态看，项目下一步不应继续推进 JSPEC 自动化抓取，而应切换为：
...
1. **JSPEC 已能重新登录，但曾触发接口风控。** 后续不得再做批量 SPA 路由跳转、CDP 调试窗口、后台脚本连续开页、自动登录或认证入口试探。
2. **已有数据已经足够支撑离线工程化第一步。** 本轮有 155 个本地响应、标准化 96 点数据、合同/结算/市场环境/公告等入口数据，先把这些变成稳定数据资产。
3. **真正缺口集中在三类交易边界数据。** 优先补能量块成交结果、能量块可买可卖限额、持仓量查询。补齐后才能讨论可买、可卖、调仓空间和策略建议。
4. **`@浏览器` 当前不可作为 JSPEC 挖掘工具。** 插件模块可加载，但浏览器运行时初始化超时；恢复前只允许在非 JSPEC 页面验证，不能回退到 CDP/脚本抓 JSPEC。
...
- 当前不再对 JSPEC 发起自动请求。
- 本次状态文档不包含 CA PIN、Cookie、x-ticket、Authorization 等敏感凭据。
...
- 不读取、不保存、不传递 CA PIN。
- 不导出私钥。
- 不保存 Cookie、x-ticket、Authorization、临时票据。
- 不批量打开 JSPEC SPA 路由。
- 不使用 CDP 调试窗口或后台脚本试探页面。
- 不点击“提交、保存、申报、撤销、确认、签章”等交易动作。
- 遇到认证失败、黑名单、暂无权限、请重新登录、风险提示，立即停止。
...
- 风控不是持续锁死，但已经证明“自动路由探测 + 后台抓包”的方式风险高。
- 后续任何“为了找接口而自动进页面”的动作都应停止。
- 后续只允许用户在普通 Chrome + CA 下人工进入页面、人工选择条件、人工导出，项目代码只做离线分析。
...
1. 不对 JSPEC 发起自动请求。
2. 不让任何凭据、票据、PIN、私钥、Cookie 进入仓库。
...
目标：把 2026-05-12 上午的尝试变成可追溯记录，避免团队后续重复踩风险路径。
...
- 状态文档已提交。
- 本计划已提交。
- 仓库没有 PIN、Cookie、Authorization、x-ticket、CA 私钥、证书私钥。
- README 或 docs 中明确：项目只做离线分析和只读辅助，不做自动交易。
...
- 标准化脚本只读取本地 raw/standard 文件，不访问 JSPEC。
...
- 提交本计划。
- 增加 `.gitignore` 安全规则。
- README 中声明只读/离线/人工导出原则。

验收：

- 仓库不包含敏感凭据。
- 文档明确禁止自动登录、CDP 批量探测、自动交易。
...
- 只输出建议，不生成交易动作。
- 对每个建议给出数据来源和缺口说明。
- 缺少能量块/持仓时，报告必须降级为“数据不足”，不能假设可交易边界。
...
- 可买可卖量不能为负；如出现负值，标记异常，不自动修正。
- 成交价为 0 或空时必须区分：真实 0、未成交、解析失败。
- 持仓和限额必须带查询日期/月份/批次，否则不得用于策略判断。
...
- 全流程没有新增 JSPEC 自动请求，没有自动登录，没有触碰 CA PIN，没有交易动作。
