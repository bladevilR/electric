...
- 缓存：入口资源使用 `workstation-v10` 版本参数，静态响应为 `cache-control: no-store`。

**v10 可读性与用户友好性复验（2026-08-30）**

- 1440、1024、768、390、320 px 均无文档级横向溢出；移动端价格预测宽表改为带提示的局部横向滚动区。
- submission 顶部持续显示“演示环境 · 样例输入”，默认真实入口不显示演示身份或 `¥24,000` 样例金额。
- 42 日同点位历史模型证据与多因素候选彻底拆分；候选明确显示“尚未独立回测”。
- 窄屏主导航拥有稳定可访问名称；当前页、日期控件和决策/审计模式均暴露程序化状态。
- 96 点曲线不再制造 96 个无操作 Tab 停靠点；证据抽屉具备对话框语义、焦点陷阱、Escape 关闭和焦点返回。
- 主操作在异步处理中跨重渲染保持禁用，避免重复采集或重复生成草稿。
- 全量测试 211 项：210 通过、0 失败、1 项因缺少本机真实业务 Excel 按设计跳过。
- 最终真实画面迭代：手机价格预测不再直接铺开 96 行形成约 5300 px 长页；结果区支持上下/左右滑动并保留吸顶表头。手机完整推导的六步目录改为横向吸顶导航，点击后高亮当前章节，锚点跳转不会遮住章节标题。
