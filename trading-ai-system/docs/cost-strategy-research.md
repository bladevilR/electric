...
当前版本会通过 `/api/settlement/reference` 扫描本地根目录下的 `2026*.xlsx`，并登记手工导出清单 `data/jspec/manual-exports/*/*/manifest.json`。这一步能把现货核对单、月度交易电量电价表、实际负荷导出目标、结算文件导出目标和持仓曲线导出目标放进同一个证据视图。

这层的定位是“复核参考”和“升级钩子”，不是训练标签。比如现货核对单里存在接近 96 点的数值行，可以帮助人工理解结算/偏差结构，但它不能替代 `actualKwh`；月度交易电量电价表只能做长期背景，不能当作日内点位结算；手工导出 manifest 如果 `files` 为空，只说明补采目标已经登记，不说明数据已经到位。

因此系统仍保持：
- `canFillActualKwh === false`，直到用户实际 96 点日电量导出文件真实存在。
- `canFillSettleAmount === false`，直到日结算或结算明细文件真实存在。
