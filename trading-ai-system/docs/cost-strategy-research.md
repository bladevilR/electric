...
这层的定位是“复核参考”和“升级钩子”。历史现货核对单中逐日 96 点的 `用电量` 和 `交易电费` 可以补历史 `actualKwh` / `settleAmount` 训练标签；但它不能代表目标交易日已经有实际负荷或结算。月度交易电量电价表只能做长期背景，不能当作日内点位结算；手工导出 manifest 如果 `files` 为空，只说明补采目标已经登记，不说明数据已经到位。
