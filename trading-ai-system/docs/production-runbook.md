...
- `/api/settlement/reference` 和“结算参考”面板会登记历史核对单、交易计算表、月度表和 manifest。历史核对单可以补历史实际负荷/结算标签；1 月现货核对单里的日前预估、偏差、交易节约费用等 96 点字段可用于复盘和后续建模；月度交易电量电价表和近三年情况可用于长期成本背景。交易计算表标准化 CSV 可以补部分月末历史实际用电、申报功率、小时持仓和操作量参考；在目标日实际负荷、目标日结算和持仓曲线为空时，不能用历史参考文件预填可执行电量或当天结算金额。
# 不可变预测复盘

准确度报告严格区分真实发布预测、历史时点重放、最终结算复盘。升级前备份
`%LOCALAPPDATA%\ElectricTradingAI\data\forecast-ledger.json`、`outcome-ledger.json`、
`point-in-time-facts.json` 和 `feature-snapshots/`。账本采用同目录临时文件加原子重命名写入；
格式升级必须保留版本并先备份，禁止覆盖旧运行 ID 或旧结果修订。

用户账本包含业务历史，不得复制进公开仓库、安装包或演示交付包。恢复时先校验 JSON 版本，
再以临时路径启动只读 API，确认真实发布与历史重放筛选互不混合。
